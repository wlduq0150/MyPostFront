...
async function getComments(postId){
	...
	for(let comment of comments)
		try{
			...
			const commentLikeBtn = $(`<button class="comment-like-btn comment-btn">&#x1F44D;</button>`)
			...
			editBtn.click(async function() {
				...
				newSubmit.click(async function(){
					await axios.put("/api/comments/${comment.id}", {content:"commentBody.text()"})
					newSubmit.remove()
			...
			commentHTML.append(`<blockquote contenteditable="false" class="comment-body">${comment.content}</blockquote>`)
